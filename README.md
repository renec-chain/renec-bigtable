# Setting up a Renec Bigtable Instance

 1. [Introduction](#renec-bigtable)
 ...
 2. [Import Renec's Bigtable Instance](#import-renec's-bigtable-instance)
 2. [Requirements](#requirements)
 2. [Restoring Missing Blocks](#restoring-missing-blocks)

## Renec Bigtable

By design an RPC node with a default `--limit-ledger-size` will store roughly 2 epochs worth of data so Renec relies on Google Cloud's Bigtable for long term storage.
The public endpoint that Renec provides https://api.mainnet-beta.renec.foundation has configured its own Bigtable instance to server requests since the Genesis Block.
This guide is meant to allow anyone to run his own Bigtable instance for long term storage in the Renec Blockchain.
...
A Warehouse node is responsible for feeding Bigtable with ledger data, so setting up one is the first thing that needs to be done in order for you to have your own Renec Bigtable instance.
...
Keeping your ledger history consistent is very important on a Warehouse node, since any gap on your local ledger will translate to a gap on your Bigtable instance, although these gaps could be potentially patched up by using `renec-ledger-tool`.
...
Before you begin:
1. [Install renec-cli](https://docs.renec.foundation/cli/install-renec-cli-tools)
...
4. [Tune your system](https://docs.renec.foundation/running-validator/validator-start#system-tuning)
...
In order to import Renec's Bigtable Instance, you'll first need to set own Bigtable instance:

1. Enable the `BigTable API` if you have not done it already, then click on the `Create Instance` inside the `Console`.
2. Name your `Instance` and then Select Storage type from HDD and SSD. Set the instance id and name to `renec-ledger`.
...
## Import Renec's Bigtable Instance

Once your Warehouse node has stored ledger data for 1 epoch successfully and you have set up your Bigtable instance as explained above, you are ready to import Renec's Bigtable to yours.
...
NOTE: Before creating the Dataflow job, you'll need to send the email address of the Service Account you created (i.e., `xxx@xxx.iam.gserviceaccount.com`) to joe@renec.foundation or axl@renec.foundation.
...
* https://explorer.renec.foundation/block/59437029: parent is 59437028
* https://explorer.renec.foundation/block/59437028: missing
* https://explorer.renec.foundation/block/59437027: child is 59437029
...
1. Download appropriate ledger data from one of these buckets based on your node's location:
    ...
    * Also note the version number in version.txt:
      * `curl https://storage.googleapis.com/mainnet-beta-ledger-us-ny5/59183944/version.txt`
        * `renec-ledger-tool 1.4.21 (src:50ebc3f4; feat:2221549166)`
2. Extract the data
...
3. Build the ledger tool from the version listed in version.txt
    * `~/renec$ git checkout 50ebc3f4` (can also checkout v1.4.21)
    * `~/renec$ cd ledger-tool && ../cargo build --release`
        * The cargo script in the renec repo uses the rust version associated with the release to solve backwards compatibility problems.
4. Check blocks
    * `~/missingBlocks/59183944$ ~/renec/target/release/renec-ledger-tool slot 59437028 -l . | head -n 2`
...
5. Upload missing block(s) to big table
    * `~/missingBlocks/59183944$ GOOGLE_APPLICATION_CREDENTIALS=<json credentials file with write permission> ~/renec/target/release/renec-ledger-tool bigtable upload 59437028 59437028 -l .`
...
6. If the previous steps produced a `SlotNotRooted` error, first run the repair-roots command.
    * `~/missingBlocks/59183944$ ~/github/renec/target/release/renec-ledger-tool repair-roots --before 59437029 --until 59437027  -l .`
        * If you get `error: Found argument 'repair-roots' which wasn't expected, or isn't valid in this context` then the ledger tool version pre-dates the repair-roots command. Add it to your local code by cherry picking `ddfbae2` or manually applying the changes from [PR #17045](https://github.com/renec-labs/renec/pull/17045/files)
